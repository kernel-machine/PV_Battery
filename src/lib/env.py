...
class NodeEnv(gym.Env):
    def __init__(self, csv_solar_data_path: str, step_s: int, stop_on_full_battery: bool = False, discrete: bool = False):
        self.device = Device(
        ...
        self.step_s = step_s
        self.solar = solar.Solar(csv_solar_data_path, scale_factor=0.18*0.28) # GTI*Surface(m2)*efficiency
        self.device.set_pv_production_current_ma(self.solar.get_solar_a(self.time_s))
    ...
    def step(self, action): #NON VA BENE CHIAMARLO OGNI TOT SECONDI, MA OGNI FINE INFERENZA
        ...
        if self.discrete:
            self.device.set_processing_rate(0 if action == 0 else 1.0)
        else:
            self.device.set_processing_rate(action[0])
        solar_current_ma = (self.solar.get_solar_a(self.time_s)/12)*1000 # I = W/V
        self.device.set_pv_production_current_ma(solar_current_ma)
        self.device.update(self.time_s)

        terminated = solar_current_ma < 0 or self.device.get_battery_percentage() == 0 or (self.device.get_battery_percentage() > 0.99 and self.stop_on_full_battery)
        nA1Sec = (self.device.last_energy_used_nah/self.step_s)
        max_nA1Sec = ((self.device.full_load_energy_na+self.device.base_load_energy_na)/self.step_s)
        ...
        # w1 = 1
        # w2 = -1
        # eloss = self.device.last_wasted_energy_mah/(self.device.last_harvested_energy_nah/1000) if self.device.last_harvested_energy_nah > 0 else 0
        # reward = w1*self.device.processing_rate + w2*eloss
        if terminated:
            print(f"Terminated after {self.time_s} seconds, with a battery level: {self.device.get_battery_percentage()}%")
    ...
    def render(self, mode="human"):
        print(f"At time {self.time_s} seconds, battery level: {self.device.get_battery_percentage()}%")
    ...
    def __get_obs(self):
        ...
        return np.array(
            [
                ...
                self.device.get_energy_consumption_ma()/((self.device.base_load_energy_na+self.device.full_load_energy_na))/1000,
    ...
    def __get_info(self):
        return {
            "Processing rate:": self.device.processing_rate,
            "Energy consumption": self.device.get_energy_consumption_ma(),
            "Battery level": self.device.get_battery_percentage(),
        }
    ...
    def get_human_uptime(self) -> str:
        ...
        output = f"{days} days, "
        output += time.strftime("%H hours, %M minutes, %S seconds", time.gmtime(self.get_uptime_s()))
        return output
