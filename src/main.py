...
args.add_argument("--discrete", default=False, action="store_true", help="Use discrete actions instead of continuous")
...
vec_env = make_vec_env(
    ...
    n_envs=args.n_env,
    seed=SEED,
    env_kwargs={"csv_solar_data_path": "../solcast2024.csv", "step_s": STEP_S, "stop_on_full_battery": True, "discrete": args.discrete},
)
...
env = NodeEnv("../solcast2025.csv", step_s=STEP_S, stop_on_full_battery=False, discrete=args.discrete)
obs = env.reset()[0]
last_action = 0
time_m = 0
min_battery_level = 1
while True:
    ...
    if time_m == (60 * 24 * 14)// (STEP_S/60):  # 14 days
        ...
        env.device.show_plot(show=False, save=True, filename=f"14_days.png")
    if done:  # 14 days
        print("Done")
        break
writer.close()
